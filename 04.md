...
```mermaid
flowchart LR
input[输入：螺丝钉] --> method1[[方法 1：锤子]] --> output1[输出/输入：钉进木板的螺丝钉] --> method2[[方法2：螺丝刀]] --> output2[输出：拧紧木板的螺丝钉]
```
...
第二天早上你没有换方法呢？你没有。你没有把方法从 “锤子” 换成 “螺丝刀”…… 因为你又不傻，你知道你自己并不适合作为 “锤子” 或者 “螺丝刀” 这两个方法中任何一个方法的 “输入”。
...
input[输入：执行之前的你] --- o((\n)) --> method[[方法：跑步]] --> output[输出：执行之后的你] --> o
```
